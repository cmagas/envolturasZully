<?php 	
	...
	function obtenerNombreClienteProveedor($idRegistro,$tipoBus)
	{
		global $con;
		...
		{
			case 1:
					$consulta="SELECT nombre FROM 4018_clientes WHERE idCliente='".$idRegistro."'";
			break;
			case 2:
					$consulta="SELECT IF(tipoEmpresa=2,nombre,CONCAT(nombre,' ',apPaterno,' ',apMaterno))AS nombre FROM 4008_proveedores WHERE idProveedor='".$idRegistro."'";
			break;
		}
	...
	function obtenerNombreEstado($cveEstado)
	{
		global $con;
		$res="";

		$consulta="SELECT estado FROM 820_estados WHERE cveEstado='".$cveEstado."'";
	...
	function obtenerNombreAlmacen($id)
	{
		global $con;
		$consulta="SELECT nombreAlmacen FROM 12_almacen WHERE idAlmacen='".$id."'";
		$res=$con->obtenerValor($consulta);
	...
	function obtenerNombreArea($id)
	{
		...
		$consulta="SELECT nombreArea FROM 2000_areas WHERE idArea='1'";
		$res=$con->obtenerValor($consulta);

		return strtoupper($res);
	}

	function obtenerGeneroUsuario($idGenero)
	{
		global $con;

		$consulta="SELECT nombreElemento FROM 1_catalogoVarios WHERE tipoElemento='2' AND valor='".$idGenero."'";
		$res=$con->obtenerValor($consulta);

		return strtoupper($res);
	}

	function obtener_cod_institucion($idEmpresa)
	{
		global $con;
		$valor="";

		$consulta="SELECT cod_unidad FROM 1002_empresa WHERE idEmpresa='".$idEmpresa."'";
		$res=$con->obtenerValor($consulta);
